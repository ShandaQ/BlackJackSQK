describe('calculatesPoints', function () {
  it('adds face values for 2-9', function () {
    var cards = [{ point: 2, suit: 'diamonds' }];
    expect(calculatesPoints(cards)).toEqual(2);
    cards = [{ point: 9, suit: 'diamonds' }];
    expect(calculatesPoints(cards)).toEqual(9);
    cards = [
      { point: 2, suit: 'diamonds' },
      { point: 9, suit: 'diamonds' }
    ];
    expect(calculatesPoints(cards)).toEqual(11);
  });

  it('values 10, Jack, Queen and King at 10 points', function () {
    expect(calculatesPoints([{ point: 10, suit: 'diamonds'} ])).toEqual(10);
    expect(calculatesPoints([{ point: 11, suit: 'diamonds'} ])).toEqual(10);
    expect(calculatesPoints([{ point: 12, suit: 'diamonds'} ])).toEqual(10);
    expect(calculatesPoints([{ point: 13, suit: 'diamonds'} ])).toEqual(10);
  });

  it('picks 1 or 11 for Ace depending if it busts', function () {
    expect(calculatesPoints([
...
describe('getCardImageUrl', function () {
  ...
  it('works for jack, queen, and king', function () {
    expect(getCardImageUrl({ point: 11, suit: 'diamonds' }))
      .toEqual('Images/jack_of_diamonds2.png');
    expect(getCardImageUrl({ point: 12, suit: 'diamonds' }))
        .toEqual('Images/queen_of_diamonds2.png');
    expect(getCardImageUrl({ point: 13, suit: 'diamonds' }))
          .toEqual('Images/king_of_diamonds2.png');
  });
});
